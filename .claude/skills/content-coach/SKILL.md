...
> **Adaptación a ESTE proyecto (Blotato por API).** Diferencias vs. el skill original:
> - El `brand-brief.md` vive en la raíz del repo. Leelo antes de empezar.
> - Cargá SIEMPRE el contexto global `_base/` (voice, hooks, platform-specs, templates,
>   publishing, accounts) además del brand-brief.
> - **Quién escribe:** el copy lo genera **Blotato**, no Claude. `post-writer` arma las
>   instrucciones y le pide a Blotato que escriba (`scripts/blotato.py write`). Claude dirige y
>   hace QC (`post-grader`); si falla, se regenera con instrucciones corregidas, no se reescribe.
> - **Transporte:** usá lo más conveniente — API directa por defecto, MCP cuando convenga o
>   falte la key, navegador solo para lo app-only. Ver `_base/transport.md`. No te quedes
>   limitado por el API.
> - **Paso visual:** entre escribir y agendar, si la pieza lleva carrusel/imagen/video,
>   invocá `visual-producer` (Blotato genera) para obtener las `mediaUrls`.
> - **Agendado por API directa:** el `post-scheduler` de este repo usa `scripts/blotato.py`
>   (no MCP). Nada se publica directo: siempre `--next-free-slot` o `--schedule`.
> El resto del método (5 ideas, hook = 50%, aprobación humana) queda igual que abajo.
