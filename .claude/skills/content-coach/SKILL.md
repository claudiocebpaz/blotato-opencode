...
> **Adaptation to THIS project (Blotato via API).** Differences vs. the original skill:
> - The `brand-brief.md` lives at the root of the repo. Read it before starting.
> - ALWAYS load the global context `_base/` (voice, hooks, platform-specs, templates,
>   publishing, accounts) in addition to the brand-brief.
> - **Who writes:** the copy is generated by **Blotato**, not Claude. `post-writer` builds the
>   instructions and asks Blotato to write (`scripts/blotato.py write`). Claude directs and
>   does QC (`post-grader`); if it fails, it's regenerated with corrected instructions, not rewritten.
> - **Transport:** use whatever is most convenient — direct API by default, MCP when convenient or
>   when the key is missing, browser only for app-only features. See `_base/transport.md`. Don't be
>   limited by the API.
> - **Visual step:** between writing and scheduling, if the piece carries a carousel/image/video,
>   invoke `visual-producer` (Blotato generates) to get the `mediaUrls`.
> - **Scheduling via direct API:** this repo's `post-scheduler` uses `scripts/blotato.py`
>   (not MCP). Nothing is published directly: always `--next-free-slot` or `--schedule`.
> The rest of the method (5 ideas, hook = 50%, human approval) stays the same as below.
